...
export class WeatherService {
  ...
  $weatherObservalble = this.weather.asObservable();
  private currentWeatherTimestamp!: number;
  private subscribers: any = {};
  private wiDataByCode: any;
  private weatherUpdateInterval = apiConfig.updateInterval['weather' as keyof typeof apiConfig.updateInterval];
  //apiConfig.measurementUnits[this.unitSystem as keyof typeof apiConfig.measurementUnits];
  ...
  //   return interval(this.weatherUpdateInterval).startWith(0)
  //     .switchMap(() =>
  //       this.http.get(
  ...
  getWeatherByLocation(latitude: number, longitude: number): Observable<any> {
    return interval(this.weatherUpdateInterval).pipe(
      startWith(0),
      switchMap(() => 
        this.http.get<any>(`${apiConfig.host}/weather?appid=${apiConfig.appId}&lat=${latitude}&lon=${longitude}&units=${this.unitSystem}`).pipe(
          map(data => {
            const weather = this.handleResponseWeatherData(data);
            this.weather.next(weather);
            return weather;
  ...
  getWeatherByCity(city: string): Observable<any> {
    return interval(this.weatherUpdateInterval).pipe(
      startWith(0),
      switchMap(() => 
        this.http.get<any>(`${apiConfig.host}/weather?appid=${apiConfig.appId}&q=${city}&units=${this.unitSystem}`).pipe(
          map(data => {
            console.log(data)
            const weather = this.handleResponseWeatherData(data);
            this.weather.next(weather);
